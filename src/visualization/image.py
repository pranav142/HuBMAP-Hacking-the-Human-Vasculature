...
def create_empty_masks(
    IMAGE_WIDTH: int = 512, IMAGE_HEIGHT: int = 512, NUM_CHANNELS: int = 3
):
    assert (
        IMAGE_WIDTH > 0 and IMAGE_HEIGHT > 0 and NUM_CHANNELS > 0
    ), "Please Enter Valid image stats"
    glomerulus_mask = np.zeros(
        (IMAGE_WIDTH, IMAGE_HEIGHT, NUM_CHANNELS), dtype=np.uint8
    )
    blood_vessel_mask = np.zeros(
        (IMAGE_WIDTH, IMAGE_HEIGHT, NUM_CHANNELS), dtype=np.uint8
    )
    unsure_mask = np.zeros((IMAGE_WIDTH, IMAGE_HEIGHT, NUM_CHANNELS), dtype=np.uint8)
    return {
        "glomerulus": glomerulus_mask,
        "blood_vessel": blood_vessel_mask,
        "unsure": unsure_mask,
    }


def fill_masks_with_annotations(
    image_id: str, masks: dict[str, np.array], polygons_df: pd.DataFrame
):
    try:
        annots = polygons_df.loc[polygons_df["id"] == image_id, "annotations"].iloc[-1]
    except error as e:
        print(
            "Could not find image please make sure you entered the correct id and that the image has a corresponding annotation"
        )
        return masks
    ...
    for annot in annots:
        ...
        color = np.random.randint(
            0, 255, size=3
        ).tolist()  # Generate a unique color for each instance
...
def combine_masks(masks: dict[str, np.array]):
    ...
    combined_image = np.concatenate(
        [glomerulus_mask, blood_vessel_mask, unsure_mask], axis=1
    )
...
def plot_masks_and_original_image(
    image_id: str, polygons_df: pd.DataFrame, base_dir: str
) -> None:
...
def display_dataset(dataset, num_examples: int = 10, figsize=(8, 8)) -> None:
    ...
    for i in range(num_examples):
        ...
        axs[i, 0].set_title(f"Image: {image.shape}")
        axs[i, 1].imshow(mask, cmap="gray")
        axs[i, 1].set_title(f"Mask:  {mask.shape}")
